...
static void *alien_load (lua_State *L, const char *libname) {
  HINSTANCE lib = NULL;
  wchar_t wlibname[MAX_PATH+1];
  wsprintf(wlibname, L"%hs", libname);
  lib = LoadLibrary(wlibname);
  if (lib == NULL) pusherror(L);
  return lib;
}

static void *alien_loadfunc (lua_State *L, void *lib, const char *sym) {
  wchar_t wsym[MAX_PATH+1];
  HINSTANCE module;
  void *f;
  module = (HINSTANCE)lib;
  if(!module) module = GetModuleHandle(NULL);
  wsprintf(wsym, L"%hs", sym);
  f = (lua_CFunction)GetProcAddress(module, wsym);
  if (f == NULL) pusherror(L);
  return f;
}
...
*/
static const ffi_abi ffi_abis[] = { FFI_DEFAULT_ABI, FFI_SYSV };
static const char *const ffi_abi_names[] = { "default", "cdecl", NULL };
